...
async function verifySignature(data, signatures, doublePackedPubkey) {
    // unpack packed pubkey to FIELD_SIZE bytes
    const packedPubkey0 = (0, utils_1.bigintToBytes)(doublePackedPubkey[0], BIGINT_SIZE);
    const packedPubkey1 = (0, utils_1.bigintToBytes)(doublePackedPubkey[1], BIGINT_SIZE);
    // combine bytes to Uint8Array
    const packedPubkey = new Uint8Array(helper_1.FIELD_SIZE);
    packedPubkey.set(packedPubkey0, 0);
    packedPubkey.set(packedPubkey1, BIGINT_SIZE);
    // unpack packed pubkey to point
    const pubkey = await (0, crypto_1.unpackPoint)(packedPubkey);
    // verify signature
    const isValid = await (0, helper_1.verifySig)(data, signatures, pubkey);
    return isValid;
}
