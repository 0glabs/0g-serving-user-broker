...
export class ServiceProcessor extends BrokerBase {
    ...
    async createTask(
        ...
        try {
            ...
            if (err instanceof Error) {
                throw new Error(
                    `Invalid JSON in trainingPath file: ${err.message}`
                )
            } else {
                throw new Error(
                    'Invalid JSON in trainingPath file: An unknown error occurred'
                )
            }
        }
        ...
        // Get model hash
        const modelHash =
            MODEL_HASH_MAP[preTrainedModelName][isTurbo ? 'turbo' : 'standard']
        await this.servingProvider.createTask(
            modelHash,
            rootHash,
            isTurbo,
            providerAddress,
    ...
    async getTaskProgress(
        ...
        return await this.servingProvider.getTaskProgress(
            providerAddress,
            serviceName,
            customerAddress
        )
